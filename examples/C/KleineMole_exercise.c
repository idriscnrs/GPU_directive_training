...
{
    // Allocation on the CPU and the GPU
    // the size is copied on the GPU
...
{
    // Allocate the forces on the CPU and GPU
    // The initialization is done on GPU 
...
Config* read_params(char* filepath)
{
    ...
    // Copy the configuration to the GPU
