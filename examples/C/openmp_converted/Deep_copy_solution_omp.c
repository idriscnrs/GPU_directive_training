...
#pragma omp target enter data map(to:arr->size) map(alloc:arr,arr->data[:size])
    return arr;
}
...
#pragma omp target enter data map(alloc:coords)
...
#pragma omp target exit data map(delete:arr->data,arr)
}
...
#pragma omp target exit data map(delete:coords)
    free(coords);
}
...
#pragma omp target update to(coords->x->data[:conf->natoms],\
            coords->y->data[:conf->natoms],coords->z->data[:conf->natoms])
    fclose(fptr);
    return coords;
}
...
#pragma omp target enter data map(alloc:hist[:maxbin],gr[:maxbin])
    
    #pragma acc parallel loop present(hist[:maxbin])
#pragma omp target teams loop map(present,alloc:hist[:maxbin])
...
#pragma omp target teams loop map(present,alloc:conf,coords,coords->x,coords->y,\
            coords->z)
    for (int j = 0; j < conf->natoms; ++j)
        #pragma acc loop private(xij,yij,zij,rij,d)
#pragma omp loop private(xij,yij,zij,rij,d)
...
#pragma omp target teams loop map(present,alloc:hist[:maxbin],gr[:maxbin])
...
#pragma omp target update from(gr[:maxbin])
...
#pragma omp target exit data map(delete:hist,gr)
    free(hist);
    free(gr);
    #pragma acc exit data delete(conf)
#pragma omp target exit data map(delete:conf)
    free(conf);
    free_coords(coords);
    return 0;
}

// Code was translated using: /home/very/bin/acc2mp -async=nowait -specify-language=C -generate-report Deep_copy_solution.c
