...
#pragma omp target exit data map(release:pop->count,pop->birth_rate,pop->death_rate)
    #pragma acc exit data delete(pop->count, pop->birth_rate, pop->death_rate)    
#pragma omp target exit data map(delete:pop->count,pop->birth_rate,pop->death_rate)
    #pragma acc exit data delete(pop)
#pragma omp target exit data map(delete:pop)
    free(pop->count);
    free(pop->birth_rate);
    free(pop->death_rate);
    free(pop);
...
#pragma omp target map(present,alloc:pop,pop->birth_rate[0:2],pop->death_rate[0:2],\
            x[0:2],dx[0:2])
    {
...
#pragma omp target data map(alloc:k1[0:2],k2[0:2],k3[0:2],k4[0:2],x_temp[0:2])\
            map(present,alloc:pop,pop->birth_rate[0:2],pop->death_rate[0:2],\
            pop->count[0:2])
        {
...
#pragma omp target enter data map(to:pred_prey->count[0:2],\
            pred_prey->birth_rate[0:2],pred_prey->death_rate[0:2])
    #pragma acc enter data copyin(pred_prey) attach(pred_prey->count, pred_prey->birth_rate, pred_prey->death_rate)
#pragma omp target enter data map(to:pred_prey)
...
#pragma omp target update from(pred_prey->count[i:1])
	}
...
// Code was translated using: /home/very/bin/acc2mp -async=nowait -specify-language=C -generate-report Deep_copy_attach_detach_solution.c
