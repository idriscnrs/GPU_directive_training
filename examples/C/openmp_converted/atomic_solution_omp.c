...
#pragma omp target teams loop map(to:shots[:nshots]) map(from:histo[0:10])
    for (size_t i=0; i<nshots; ++i)
    {
...
// Code was translated using: /home/very/bin/acc2mp -async=nowait -specify-language=C -generate-report atomic_solution.c
