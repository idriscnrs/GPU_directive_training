...
#pragma omp target teams loop map(present,alloc:forces,dyn,conf)
...
#pragma omp target teams loop reduction(+:potential_energy) map(present,alloc:forces,\
            dyn,conf)
    for (size_t i=0; i<conf->NAtoms; ++i)
        #pragma acc loop private(xij, yij, zij, rij2)
#pragma omp loop private(xij,yij,zij,rij2)
...
double stat_forces(Forces* forces, Config* conf)
{
    ...
    double Fnorm = 0.;
    double F = 0.;
    #pragma acc parallel loop reduction(min:Fmin) reduction(max:Fmax) reduction(+:Fnorm)\
...
#pragma omp target teams loop reduction(min:Fmin) reduction(max:Fmax)\
            reduction(+:Fnorm) map(tofrom:Fmin,Fmax,Fnorm)
...
{
    #pragma acc parallel loop present(conf, dyn, forces, dyn->vx, dyn->vx->data[:conf->NAtoms])\
                              ...
                              present(forces->Fy,forces->Fy->data[:conf->NAtoms])\
                              present(forces->Fz,forces->Fz->data[:conf->NAtoms])
#pragma omp target teams loop map(present,alloc:conf,dyn,forces,dyn->vx,\
            dyn->vx->data[:conf->NAtoms])
    for (size_t i=0; i < conf->NAtoms; ++i)
    {
    ...
    #pragma acc parallel loop present(conf, forces, dyn)\
                              ...
                              present(forces->Fy,forces->Fy->data[:conf->NAtoms])\
                              present(forces->Fz,forces->Fz->data[:conf->NAtoms])
#pragma omp target teams loop map(present,alloc:conf,forces,dyn)
    for (size_t i=0; i < conf->NAtoms; ++i)
    {
...
#pragma omp target enter data map(to:ar->size) map(alloc:ar,ar->data[:size])
    return ar;
}
...
#pragma omp target exit data map(delete:forces)
    free(forces);
}
...
#pragma omp target exit data map(delete:dyn)
    free(dyn);
}
...
#pragma omp target update to(ar->data[:size])
    }
    else
    {
        #pragma acc update self(ar->data[:size])
#pragma omp target update from(ar->data[:size])
    }
}
...
#pragma omp target enter data map(alloc:dyn)
...
#pragma omp target enter data map(alloc:forces)
    forces->Fx = allocate_array(conf->NAtoms);
    forces->Fy = allocate_array(conf->NAtoms);
    forces->Fz = allocate_array(conf->NAtoms);
    #pragma acc parallel loop present(forces, conf,forces->Fx, forces->Fy, forces->Fz, forces->Fx->data[:conf->NAtoms])\
                              present(forces->Fy->data[:conf->NAtoms])\
                              present(forces->Fz->data[:conf->NAtoms])
#pragma omp target teams loop map(present,alloc:forces,conf,forces->Fx,forces->Fy,\
            forces->Fz,forces->Fx->data[:conf->NAtoms])
    for (size_t i=0; i<conf->NAtoms; ++i)
    {
        forces->Fx->data[i] = 0.;
        forces->Fy->data[i] = 0.;
...
#pragma omp target enter data map(to:conf)
    return conf;
}
...
double berendsen_thermostat(dynamic* dyn, Config* conf)
{
    ...
    #pragma acc parallel loop present(dyn) reduction(+:kinetic_E) \
                          ...
                          present(dyn->vy, dyn->vy->data[:conf->NAtoms])\
                          present(dyn->vz, dyn->vz->data[:conf->NAtoms])
#pragma omp target teams loop reduction(+:kinetic_E) map(tofrom:kinetic_E)\
            map(present,alloc:dyn)
    for (size_t i=0; i<conf->NAtoms; ++i)
    ...
    #pragma acc parallel loop present(dyn)\
                          ...
                          present(dyn->vy, dyn->vy->data[:conf->NAtoms])\
                          present(dyn->vz, dyn->vz->data[:conf->NAtoms])
#pragma omp target teams loop map(present,alloc:dyn)
    for (size_t i=0; i < conf->NAtoms; ++i)
    {
...
// Code was translated using: /home/very/bin/acc2mp -async=nowait -specify-language=C -generate-report Hands_on_LJ_solution.c
