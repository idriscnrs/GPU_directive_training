...
#pragma omp target data map(from:histo[:10]) map(alloc:shots[:nshots])
    {
        #pragma acc host_data use_device(shots) 
#pragma omp target update from(shots)
        {
...
#pragma omp target teams loop map(present,alloc:shots[:nshots])
...
// Code was translated using: /home/very/bin/acc2mp -async=nowait -specify-language=C -generate-report Using_CUDA_random_example.c
