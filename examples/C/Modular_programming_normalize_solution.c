...
{
    double N = 0.0;
    #pragma acc loop vector reduction(+:N)
...
double check(double* restrict A, size_t num_rows, size_t num_cols)
{
    ...
    double s = 0.0;
    #pragma acc parallel loop gang present(A[0:num_rows*num_cols]) reduction(+:s)
...
int main(void)
{
    ...
    #pragma acc parallel present(A[0:size*size])
    {
    ...
    #pragma acc exit data delete(A[0:size*size])
    free(A);
}
